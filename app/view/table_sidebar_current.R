  ...
  tagList(
    ...
    dropdownButton(
      pickerInput(ns("mgi_viability_filter"),
                  ...
                  multiple = TRUE,
                  selected = df$mgi_viability,
                  options = pickerOptions(
                    actionsBox = TRUE,
                    showTick = TRUE,
                    width = "100%"
                  ),
                  choices = na.omit(df$mgi_viability)),
      pickerInput(ns("impc_viability_filter"),
                  ...
                  choices = na.omit(df$impc_viability)),
...
server <- function(id, filtered_df) {
  moduleServer(id, function(input, output, session) {
    ...
    filtered_table <- reactive({
      ...
      # user_chosen_table_data4 <- table_manage$filterMouseModels(
      #   user_chosen_table_data3,
      #   input$mgi_viability_filter,
      #   input$impc_viability_filter
      #   ,
      #   input$impc_phenotypes_heterozygote_filter,
      #   input$impc_phenotypes_hemizygote_filter,
      #   input$impc_phenotypes_homozygote_filter
      # )
      })
    })
